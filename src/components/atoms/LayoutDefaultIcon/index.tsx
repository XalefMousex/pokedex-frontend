import { type LayoutDefaultIconProps } from './types';

export const LayoutDefaultIcon = ({ ...props }: LayoutDefaultIconProps) => (
  <svg
    viewBox="0 0 79.86 51.14"
    data-name="con-layout-default"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <path
      opacity={0.75}
      d="M39.22 15.99h-8.16c-.79 0-1.43-.67-1.43-1.5s.64-1.5 1.43-1.5h8.16c.79 0 1.43.67 1.43 1.5s-.64 1.5-1.43 1.5z"
    />
    <rect
      x={29.63}
      y={18.39}
      rx={1.36}
      ry={1.36}
      width={16.72}
      height={2.73}
      opacity={0.5}
    />
    <path
      opacity={0.9}
      d="M75.1 6.68v1.45c0 .63-.49 1.14-1.09 1.14H30.72c-.6 0-1.09-.51-1.09-1.14V6.68c0-.62.49-1.14 1.09-1.14h43.29c.6 0 1.09.52 1.09 1.14z"
    />
    <rect
      x={29.63}
      y={24.22}
      rx={2.11}
      ry={2.11}
      width={21.8}
      opacity={0.4}
      height={19.95}
    />
    <g strokeLinecap="round" strokeMiterlimit={10}>
      <rect
        x={61.06}
        y={38.15}
        rx={0.33}
        ry={0.33}
        width={2.01}
        height={3.42}
        opacity={0.32}
      />
      <rect
        x={56.78}
        y={34.99}
        rx={0.33}
        ry={0.33}
        width={2.01}
        height={6.58}
        opacity={0.44}
      />
      <rect
        x={65.17}
        y={32.86}
        rx={0.33}
        ry={0.33}
        width={2.01}
        height={8.7}
        opacity={0.53}
      />
      <rect
        x={69.55}
        y={29.17}
        rx={0.33}
        ry={0.33}
        width={2.01}
        height={12.4}
        opacity={0.66}
      />
    </g>
    <g opacity={0.5}>
      <circle r={7.5} cx={63.17} cy={18.63} />
      <path d="M63.17 11.63c3.86 0 7 3.14 7 7s-3.14 7-7 7-7-3.14-7-7 3.14-7 7-7m0-1c-4.42 0-8 3.58-8 8s3.58 8 8 8 8-3.58 8-8-3.58-8-8-8z" />
    </g>
    <g opacity={0.74}>
      <path d="M64.05 18.13l3.38-5.67c.93.64 1.7 1.48 2.26 2.47.56.98.89 2.08.96 3.21h-6.6z" />
      <path d="M67.57 13.19a6.977 6.977 0 012.52 4.44h-5.17l2.65-4.44m-.31-1.43l-4.1 6.87h8c0-1.39-.36-2.75-1.04-3.95a8.007 8.007 0 00-2.86-2.92z" />
    </g>
    <g strokeLinecap="round" strokeMiterlimit={10}>
      <rect
        rx={2}
        ry={2}
        x={5.84}
        y={5.02}
        height={40}
        width={19.14}
        opacity={0.8}
      />
      <g stroke="#fff">
        <path
          fill="none"
          opacity={0.72}
          strokeWidth="2px"
          d="M9.02 17.39L21.25 17.39"
        />
        <path
          fill="none"
          opacity={0.48}
          strokeWidth="2px"
          d="M9.02 24.6L19.54 24.6"
        />
        <path
          fill="none"
          opacity={0.55}
          strokeWidth="2px"
          d="M9.02 20.88L18.4 20.88"
        />
        <circle r={2.54} cy={9.91} cx={10.98} fill="#fff" opacity={0.8} />
        <path
          fill="none"
          opacity={0.8}
          strokeWidth="2px"
          d="M15.53 8.65L21.25 8.65"
        />
        <path fill="none" opacity={0.6} d="M15.32 11.3L20.38 11.3" />
      </g>
    </g>
  </svg>
);
